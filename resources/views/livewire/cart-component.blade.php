        ...
        <div class=" main-content-area">
                ...
                <ul class="products-cart">
                    ...
                    <li class="pr-cart-item">
                        <div class="product-image">
                            <figure>
                                <img src="{{('assets/images/products')}}/{{$item->model->image}}"
                                    alt="{{$item->model->name}}">
                            </figure>
                        </div>
                        <div class="product-name">
                            <a class="link-to-product" href="{{
								...
								route('product.details', ['slug' => $item->model->slug])}}">{{$item->model->name}}</a>
                        </div>
            ...
            <div class="summary">
                <div class="order-summary">
                    <h4 class="title-box">Rendelés Összegzése</h4>
                    <p class="summary-info"><span class="title">Részösszeg</span><b
                            class="index">${{Cart::subtotal()}}</b></p>
                    @if(Session::has('coupon'))
                    <p class="summary-info"><span class="title">Diszkont({{Session::get('coupon')['code']}})<a href="#"
                                wire:click.prevent="removeCoupon"><i class="fa fa-times text-danger"></i></a></span><b
                            class="index">${{$discount}}</b></p>
                    <p class="summary-info"><span class="title">Subtotal diszkonttal</span><b
                    ...
                    <p class="summary-info total-info "><span class="title">Fizetendő</span><b
                            class="index">${{$totalAfterDiscount}}</b></p>
                    @else

                    <p class="summary-info"><span class="title">Adó</span><b class="index">${{Cart::tax()}}</b></p>
                    <p class="summary-info"><span class="title">Szállítási díj</span><b class="index">Free Shipping</b>
                    </p>
                    <p class="summary-info total-info "><span class="title">Fizetendő</span><b
                ...
                <div class="checkout-info">
                    <label class="checkbox-field">
                        <input class="frm-input " name="have-code" id="have-code" value="1" type="checkbox"
                            wire:model="haveCouponCode"><span>Van kuponkódom</span>
                    </label>

                    @if ($haveCouponCode == 1)
                    <div class="summary-item">
                        <form wire:submit.prevent="applyCouponCode">
                            <h4 class="title-box">Kupon kód</h4>
                            ...
                            @endif
                            <p class="row-in-form">
                                <label for="coupon-code">Add meg a kuponkódod:</label>
                                <input type="text" name="coupon-code" wire:model="couponCode">
                            </p>
                            <button type="submit" class="btn btn-small">Rendben</button>
                        </form>
                    </div>
                    ...
                    <a class="btn btn-checkout" href="#" wire:click.prevent="checkout">Pénztárhoz</a>
                    <a class="link-to-shop" href="shop.html">Continue Shopping<i class="fa fa-arrow-circle-right"
                            aria-hidden="true"></i></a>
                </div>
                <div class="update-clear">
                    <a class="btn btn-clear" href="#" wire:click.prevent="destroyAll()">Összes termék törlése</a>
            ...
            <div class="text-center" style="padding: 30px 0;">
                ...
                <a href="/shop" class="btn btn-success">Vásárlás folytatása</a>
            </div>
